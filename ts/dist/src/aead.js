...
import { encodeProfile, getPrimeAtIndex, defaultProfile } from './multiplicity';
...
function _nonceTag(profile) {
    const prime = getPrimeAtIndex(profile.prime_index);
    const tag = Buffer.alloc(4);
    tag.writeUInt32BE(prime, 0);
    return tag;
}
export function encryptAEAD({ key, nonce, plaintext, aad, profile }) {
    if (!plaintext)
...
export function decryptAEAD({ key, nonce, ciphertext, authTag, aad, profile }) {
    if (!ciphertext || !authTag)
        throw new Error('Ciphertext and authTag required for decryption');
    const p = profile ?? defaultProfile();
    const profileBuf = encodeProfile(p);
    const fullAad = Buffer.concat([aad, profileBuf, _nonceTag(p)]);
