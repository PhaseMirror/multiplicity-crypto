...
import {
  ...
  deriveDirectionalKeysFrom,
  deriveDirectionalKeysWithSalt,
  deriveNoncePrefix,
  deriveNoncePrefixes,
  encodeBootstrap,
  encodeClassical,
...
const vectorContextHash = Buffer.from(
...
describe('frame codec', () => {
  ...
  it('rejects malformed headers, lengths, types, and tags', () => {
    ...
    expect(() => encodeFrame({ type: CLASSICAL_MESSAGE, sequence: 0, payload: Buffer.alloc(0) })).toThrow(
      /requires authenticationTag/
    );
    expect(() =>
      encodeFrame({
        type: CLASSICAL_MESSAGE,
...
describe('HMAC authentication', () => {
  it('matches the normative HMAC input and tag vector', () => {
    ...
    const frame: FrameInput = { type: CLASSICAL_MESSAGE, sequence: 1, payload: Buffer.from([1, 2, 3, 4]) };
  ...
  it('rejects header, payload, and tag tampering with constant-time comparison', () => {
    ...
    expect(constantTimeEqual(computeAuthenticationTag(zeroKey, frame), frame.authenticationTag as Buffer)).toBe(true);
...
describe('RFC 5869 and directional derivation', () => {
  it('matches the PRK, directional key, and nonce-prefix vectors', () => {
    expect(hkdfExtract(Buffer.alloc(32, 0), zeroSalt).toString('hex')).toBe(
      '33ad0a1c607ec03b09e6cd9893680ce210adf300aa1f2660e1b22e10f170f92a'
    );

    const keys = deriveDirectionalKeys(Buffer.alloc(32, 0), vectorContextHash);
    expect(keys.K_enc_A2B.toString('hex')).toBe(
      'dd70c9f5110ea586dac2ba20569481ad03d8df346c9d17cd78cbf70a9cd2c9e5'
    );
    expect(keys.K_enc_B2A.toString('hex')).toBe(
  ...
  it('implements HKDF expand independently of the convenience wrapper', () => {
    ...
    expect(expanded.equals(Buffer.from(expected as ArrayBuffer))).toBe(true);
